// Package builtinchecks embeds gwlint's default check configs, mirroring
// kube-linter's own pkg/builtinchecks. A check.Template registration alone
// does not make kube-linter's engine run a check by default: the engine
// needs a config.Check entry (name, template, scope, remediation) loaded
// into a checkregistry.CheckRegistry, which is what List/LoadInto provide.
...
// LoadInto loads gwlint's built-in checks into the registry.
...
// List lists gwlint's built-in checks.
